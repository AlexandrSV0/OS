...
static void put_begin(game_t *g, player_t* player){
	bool pl_exist = false;
	if (g != NULL) {
	...
	printf("%s%s%c ", g->name, g->winner_idx < 0 ? "" : g->winner_idx == 0 ? " winner" : " loser", g->active_player >= 0 ? '#' : '>');
	fflush(stdout);
}

static game_t* CreateGame(player_t *player, char *name, int max_players){
	int num;
...
static game_t* JoinGame(player_t *player, char *name){
...
void process_cmd(int fd_r, int fd_w){
	...
	for(;;){
		...
		if(strncmp(cmd, "join", 4) == 0){
			...
			if (game->pl_number >= game->max_players) game->active_player = 0;
			continue;
		} else
		if(cmd[0] == 'a') {
